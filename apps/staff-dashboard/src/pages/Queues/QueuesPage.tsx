...
export default function QueuesPage() {
  ...
  const getAriaSort = (field: QueueSortField) => (sortBy === field ? 'descending' : 'none');

  return (
    <div className="space-y-4">
      <section aria-labelledby="queues-filter-heading" className="rounded-2xl border border-navy-border bg-navy-card p-5 shadow-panel">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <p className="text-sm font-medium uppercase tracking-[0.16em] text-slate-400">Queue Monitoring</p>
            <h2 id="queues-filter-heading" className="mt-1 text-lg font-semibold text-slate-50">
              Real-time stall throughput and service pressure
            </h2>
          </div>
          <div className="flex flex-wrap gap-2">
            {filterChips.map((chip) => {
              const label = chip === 'MERCH' ? 'merchandise' : chip.toLowerCase();
              return (
                // ACCESSIBILITY: Names each queue filter option.
                <button
                  key={chip}
                  type="button"
                  onClick={() => setFilter(chip)}
                  className={`rounded-full border px-4 py-2 text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 ${
      ...
      <section aria-labelledby="queues-table-heading" className="rounded-2xl border border-navy-border bg-navy-card shadow-panel">
        <h2 id="queues-table-heading" className="sr-only">
          Queue status table
        </h2>
        {longestQueue ? (
          ...
          <table className="min-w-full text-left">
            <thead className="border-b border-navy-border bg-navy-elevated/70">
              <tr className="text-sm uppercase tracking-[0.14em] text-slate-400">
                <th className="px-5 py-4" aria-sort={getAriaSort('name')}>
                  {/* ACCESSIBILITY: Names the sort control for stall names. */}
                  <button
                    type="button"
                    onClick={() => setSortBy('name')}
                    className="inline-flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    aria-label="Sort queues by stall name"
                  >
                    Stall
                    <ChevronDown className="h-4 w-4" />
                  </button>
                </th>
                <th className="px-5 py-4" aria-sort={getAriaSort('length')}>
                  {/* ACCESSIBILITY: Names the sort control for queue length. */}
                  <button
                    type="button"
                    onClick={() => setSortBy('length')}
                    className="inline-flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    aria-label="Sort queues by queue length"
                  >
                    Queue Length
                    <ArrowDownWideNarrow className="h-4 w-4" />
                  </button>
                </th>
                <th className="px-5 py-4" aria-sort={getAriaSort('wait')}>
                  {/* ACCESSIBILITY: Names the sort control for wait times. */}
                  <button
                    type="button"
                    onClick={() => setSortBy('wait')}
                    className="inline-flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    aria-label="Sort queues by wait time"
                  >
                    Wait Time
                    <ArrowDownWideNarrow className="h-4 w-4" />
                  </button>
                </th>
                <th className="px-5 py-4" aria-sort={getAriaSort('status')}>
                  {/* ACCESSIBILITY: Names the sort control for queue status. */}
                  <button
                    type="button"
                    onClick={() => setSortBy('status')}
                    className="inline-flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    aria-label="Sort queues by open or closed status"
                  >
                    Status
                    <ArrowDownWideNarrow className="h-4 w-4" />
                  </button>
              ...
              {loading
                ? Array.from({ length: 6 }, (_, index) => (
                    <tr key={index} className="border-b border-navy-border">
                      <td className="px-5 py-4" colSpan={7}>
                        <div className="h-14 animate-pulse rounded-2xl bg-slate-800/70" />
                      </td>
                    </tr>
                  ))
                : filteredQueues.map((queue) => {
                    ...
                    return (
                      <tr
                        key={queue.queueId}
                        className={`border-b border-navy-border text-sm transition-colors duration-700 ${flashRows[queue.queueId] ? 'bg-amber-500/12' : 'bg-transparent'}`}
                      >
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            <span className="flex h-10 w-10 items-center justify-center rounded-2xl border border-navy-border bg-navy-elevated text-slate-200">
                        ...
                        <td className="px-5 py-4 tabular-nums text-slate-200">{queue.currentLength}</td>
                        <td className="px-5 py-4">
                        ...
                        <td className="px-5 py-4 tabular-nums text-slate-300">{normalizedLoad}%</td>
                        <td className="px-5 py-4 text-slate-400">{formatDistanceToNowStrict(new Date(queue.lastUpdated), { addSuffix: true })}</td>
                        <td className="px-5 py-4">
                          {/* ACCESSIBILITY: Names the override action for each queue row. */}
                          <button
                            type="button"
                            className="inline-flex items-center gap-2 rounded-xl border border-venue-blue/40 bg-venue-blue/15 px-3 py-2 text-sm font-medium text-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                            aria-label={`Override wait time for ${queue.stallName}`}
                          >
